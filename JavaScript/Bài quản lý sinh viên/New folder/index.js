...
function deleteContact(){
  ...
  for(var i = 0; i < contacts.length; i++){
      if(i == del ){
        var newContact2 = contacts.splice(del, 1);
      }
  }
  if(newContact2){
  fs.writeFileSync('./data.json',JSON.stringify(contacts));
...
function searchContact(){
  ...
  switch(search){
    case '1':
    var nameFind = readlineSync.question('Please write name you want find: ');
    for(var i = 0; i < contacts.length; i++){
      if(contacts[i].name.includes(nameFind)){
        console.log(contacts[i]);
      }
    };
    break;     
    case '2':
    var phoneFind = readlineSync.question('Please write phone you want find: ');
    for(var i = 0; i < contacts.length; i++){
      if(contacts[i].phone.toString().includes(phoneFind)){
        console.log(contacts[i]);
      }
    };
    break;     
